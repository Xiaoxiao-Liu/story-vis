...
class Fenye(object):
    ...
    def __sql_read(self,title):  # 创建读取数据库的函数
        ...
        self.emo_cursor.execute('SELECT tc.text_content FROM text_table t,text_content_table tc where t.text_id=tc.text_id and t.text_title="%s";'%title)
    ...
    def qinggan(self,title,page):    # 创建分页and分析情感的函数，需要输入故事名称，页数
        fen_data=self.fen(title,page)  # 调用分页函数
        #print(fen_data)
        self.zheng_sum = 0
        self.fu_sum = 0
...
#
#
# print(a.qinggan('狐狸和乌鸦的故事',20))
# print(a.max())
# print(a.min())

    # def run(self, title, page):
    #     pages = []
    #     pro_nums=[]
    #     for i in self.qinggan(title, page):
    #         pages.append(i)
            # print(i)
            # pro_num = []
            # for x in i:  # 循环数据，将数据做归一化处理，处理结果控制在0-1的范围里面
            #     if x>=0:
            #         all = round((float(x) - 0) / float(max(i) - 0),
            #                 3)  # 归一化公式，并将值保留小数点后三位
            #     else:
            #         all = -(round((float(x) - min(i)) / float(0 - min(i)),
            #                     3) ) # 归一化公式，并将值保留小数点后三位)
            #    # print(all)
            #     pro_num.append(all)
            #print(pro_num)
            # pro_nums.append(pro_num)
          # 调用最大值最小值的函数
        # max_num=self.max()
        # min_num=self.min()
        # print(pages)
...
# a=fenye()
# a.fen('狐狸和乌鸦的故事',50)
# a.qinggan('狐狸和乌鸦的故事',50)
# for i in a.qinggan('狐狸和乌鸦的故事',10):
#     print(i)
#
# a.max_and_min()   # 调用最大值最小值的函数
# print(a.qinggan_max)
# print(a.qinggan_min)
