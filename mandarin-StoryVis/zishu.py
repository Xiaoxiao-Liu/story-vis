...
class Wordxi(object):
    ...
    def __sql_read(self, title):  # 创建读取数据库的函数
        ...
        # story_data=tuple(mode.models_mysql(title))
        # print("故事内容：",story_data)
        # 返回元组
        return story_data
    ...
    def cixing(self, title, page):  # 创建分页and分析情感的函数，需要输入故事名称，页数
        ...
        for i in range(1, len(fen_data[0]) + 1):  # 将分页的故事内容一页一页的进行分词，

            re_tetx = re.sub(r'[，|。|、|：|“|”|？|！|;|\,|\.|\"|\"|:]*', '', fen_data[0]['page_' + str(i)])  # 去除标点符号
...
# a = Word_ci()  # 创建对象
# print(a.fen('狐狸和乌鸦的故事',50))
# print(a.cixing('狐狸和乌鸦的故事',50))
# print(a.page_cixing())

# a.fen('狐狸和乌鸦的故事',50)
# a.qinggan('狐狸和乌鸦的故事',50)
# for i in a.qinggan('狐狸和乌鸦的故事',20):
#     print(i)
